...
class PolarGraphExample(Scene):
    def construct(self):
        ...
        def get_integral_area():
            theta_max = tracker.get_value()
            points = [origin]

            # 生成曲线上的点

            for t in np.linspace(0, theta_max, 50):
                try:
                    r = r_func(t)
                    x = r*np.cos(t)
                    y = r*np.sin(t)
                    points.append(polar_plane.c2p(x, y))
                except:
                    continue
        ...
        # 创建动态区域
        integal_area = always_redraw(get_integral_area)
