...
# 获取函数表达式并解析
func_input = input("请输入显式函数 y = f(x)（例如 x ** 2）：")
...
class FunctionPlot(Scene):
    def construct(self):
        ...
        # 创建动态积分区域
        tracker = ValueTracker(x_left) # type: ignore # 目前暂定为人为定义的左侧区间
        intergral_area = always_redraw(
            lambda: axes.get_area(
                graph,
                x_range=(x_left, tracker.get_value()),
                color=BLUE,
                opacity=0.5
            )
        )
        
        self.add(intergral_area)
        self.play(
            tracker.animate.set_value(x_right),
            rate_func=linear,
            runtime=8
        )
        self.wait(2)
